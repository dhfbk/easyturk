      ...
      <div tabindex="-1" id="modal" class="bg-white rounded-lg w-5/6 max-w-3xl max-h-80 overflow-y-auto" @click.stop>
        <div class="flex flex-col p-4">
          ...
          <div class="flex flex-col sm:flex-row items-center content-center w-full pb-1">
            <label class="block tracking-wide text-gray-900 text-md font-bold mb-2 sm:mb-0 mr-2" for="hitNum"
            ...
            <div class="relative">
              <input
                ...
                class="appearance-none focus:border-blue-500 hover:border-blue-500 block w-full sm:max-w-xs border border-gray-200 rounded py-2 pl-4 pr-16"
                id="hitNum"
                type="number"
                min="1"
                :max="hitMax"
  ...
  methods: {
    ...
    calculatePrice(mode) {
      ...
      if (this.priceData.assignment >= 10) {
        this.mtFee =
          Math.round((this.hitNum * this.priceData.assignment * this.priceData.reward * 0.4 + 0.00001) * 100) / 100 <
          0.01
            ? 0.01
            : Math.round((this.hitNum * this.priceData.assignment * this.priceData.reward * 0.4 + 0.00001) * 100) / 100
      } else {
        this.mtFee =
          Math.round((this.hitNum * this.priceData.assignment * this.priceData.reward * 0.2 + 0.00001) * 100) / 100 <
          0.01
            ? 0.01
            : Math.round((this.hitNum * this.priceData.assignment * this.priceData.reward * 0.2 + 0.00001) * 100) / 100
      }
      ...
      if (this.qualifications.master == 0) {
        ...
        this.masterFee =
          Math.round((this.hitNum * this.priceData.assignment * this.priceData.reward * 0.05 + 0.00001) * 100) / 100
      }
    },
    toggleModal() {
